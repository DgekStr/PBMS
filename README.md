...
- Python 3;
- `pvesh` и права `root`;
- `ssh` (openssh-client) для чтения каталогов backup-файлов с других нод кластера;
...
```bash
apt install python3 msmtp openssh-client
```
...
```bash
apt update
apt install -y git python3 msmtp openssh-client
...
По умолчанию коллектор ищет backup-файлы в следующих хранилищах. Если один и тот же архив найден в нескольких каталогах, в отчёт попадает файл с самой новой датой изменения среди всех каталогов, а `path` всегда показывает фактический каталог выбранного файла.
...
Для каждого VM/LXC PBMS проверяет стандартный каталог `PBMS_BACKUP_DIR` и все дополнительные пути, настроенные для соответствующей ноды, затем выбирает самый новый файл с префиксом `vzdump-qemu-ID-` или `vzdump-lxc-ID-`. Поэтому более свежий архив на другом носителе будет показан с его фактической датой и правильным путём хранения. Если рядом существует `.log`, последние 20 000 символов проверяются по словам `error`, `failed`, `failure`, `critical` и `unable`. Дополнительно проверяется размер архива: по умолчанию backup меньше `100 MB` получает статус `TOO_SMALL`, красную подсветку в HTML и предупреждение «Размер бэкапа на VM/LXC<ID> слишком мал, возможно повреждение. Нужна проверка!». Порог настраивается через `PBMS_MIN_BACKUP_SIZE_MB`.

Каталоги, указанные в `NODE_BACKUP_PATHS` (например `/mnt/hdd2/dump` на `pve2`), часто являются локальными дисками конкретной ноды и не видны с других нод кластера. В этом случае коллектор читает список файлов по `ssh` непосредственно с ноды-владельца и для проверки лога бэкапа также обращается к нему через `ssh`. Свежий дамп с `pve2` будет найден и показан в отчёте, даже если сбор запускается на `pve`. Для работы требуется доступ по `ssh` от `root` между нодами кластера.
...
python3 tests/test_collector_paths.py
```

Тест проверяет генерацию HTML, baseline, проверку минимального размера backup и наличие данных mock-объектов. Отправка почты в тестовом окружении намеренно может завершиться ошибкой, поскольку MTA не требуется для проверки сбора и генерации. Для email используется полный HTML-отчёт с предупреждениями о малых и устаревших архивах, а Mattermost получает список изменений, статистику и табличное представление актуального отчёта. Дополнительный тест [`tests/test_collector_paths.py`](tests/test_collector_paths.py) подтверждает выбор самого нового архива среди стандартного и дополнительных носителей.
